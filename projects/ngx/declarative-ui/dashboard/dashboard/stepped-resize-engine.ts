...
export class SteppedResizeGridStackEngine extends GridStackEngine {
  ...
  private moveNodeCheckWithSteppedResize(
    ...
    if (!node.id || opts.w === undefined) return super.moveNodeCheck(node, opts);
  ...
  private moveNodeCheckWithZFlowDrag(
    ...
    let insertionSlotInRow: number;
    if (rect) {
      const rectLeft =
        (rect as unknown as { left?: number }).left ?? rect.x ?? 0;
      const rectWidth =
        (rect as unknown as { width?: number }).width ?? rect.w ?? cellWidth;
      const dragCenterX = rectLeft + rectWidth / 2;
      insertionSlotInRow = resolveInsertionSlotFromRow(
        dragCenterX,
        rowNodes,
        node.id as string,
        cellWidth,
      ...
      insertionSlotInRow = rowNodes.filter((n) => {
    ...
    const newOrdered = sortNodesByZFlowOrder(nodes);
    const newProjected = projectZFlowLayout(newOrdered, this.column);
    applyProjectedLayout(nodes, newProjected);
    notifyEngine(this);
