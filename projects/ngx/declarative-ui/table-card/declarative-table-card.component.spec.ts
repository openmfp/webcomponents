...
describe('DeclarativeTableCard', () => {
  ...
  // -------------------------------------------------------------------------
  // 5. searchConfig — renders mfp-table-card-search
  // -------------------------------------------------------------------------

  describe('searchConfig rendering', () => {
    it('does not render mfp-table-card-search when searchConfig is absent', () => {
      const { fixture } = setup();
      expect(root(fixture).querySelector('mfp-table-card-search')).toBeNull();
    });

    it('renders mfp-table-card-search when searchConfig is provided', () => {
      const { fixture } = setup({
        searchConfig: { placeholder: 'Search pods…' },
      });
      fixture.detectChanges();
      expect(root(fixture).querySelector('mfp-table-card-search')).not.toBeNull();
    });
  });
